...
class DataService {
  ...
  // Get fishmeal plant locations (~324 plants). Static dataset generated by
  // generate-fishmeal-plants.js; only available as JSON, no GraphQL collection.
  async getFishmealPlants() {
    const cacheKey = 'json:/fishmeal-plants.json';
    if (this.cache[cacheKey]) {
      return this.cache[cacheKey];
    }
    const data = await this.fetchFromJSON('/fishmeal-plants.json');
    this.cache[cacheKey] = data;
    return data;
  }
